...
- Online Natural and Linux Piper synthesis are both kept at canonical/native 1.0x timing. The **Speed** control changes only finished-audio playback rate in the browser (with pitch preservation), so user listening speed does not alter TTS generation or add a prosody confound.
